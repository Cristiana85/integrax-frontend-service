...
export class HybridPathUtils {
  ...
  static pathRefactoring(
    ...
    let d = undefined;
    if (userPath) {
      const orderedPath = HybridPathUtils.removeOverlappedSegments(userPath);
      return orderedPath;
    } else {
      ...
      if (path.length > 0) {
        ...
        const orderedPath = HybridPathUtils.removeOverlappedSegments(path);
  ...
  static removeOverlappedSegments(path: joint.g.Point[]): joint.g.Point[] {
    ...
    for (var index = 0; index < path.length - 1; index++) {
      ...
      if (index === 0) {
        lastsegmentOrientation = HybridPathUtils.getPathOrientation(p0, p1);
        resegment.push(new joint.g.Point({ x: p0.x, y: p0.y }));
      } else if (index > 0) {
        const curentOrientation: ORIENTATION = HybridPathUtils.getPathOrientation(p0, p1);
